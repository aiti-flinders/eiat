employment_industry_fy <- function(financial_year) {
  ...
  aitidata::employment_by_industry %>%
    dplyr::filter(dplyr::between(date, date_left, date_right),
           gender == "Persons", age == "Total (age)",
           state == "Australia",
           indicator %in% c("Employed full-time", "Employed total"),
           industry != "Total (industry)") %>%
    dplyr::left_join(abscorr::anzsic, by = c("industry" = "anzsic_division")) %>%
    dplyr::distinct(indicator, anzsic_division_code, value) %>%
    dplyr::group_by(indicator, anzsic_division_code) %>%
    dplyr::summarise(value = mean(value)) %>%
    tidyr::pivot_wider(names_from = anzsic_division_code) %>%
    dplyr::ungroup() %>%
    dplyr::rename(from_anzsic = indicator) %>%
    dplyr::mutate(total_industry_uses = rowSums(across(c(2:20))))
}
...
fte_employment <- function(data) {
  ratio <- industry_industry_flows_19 %>%
    ...
    tidyr::pivot_wider(names_from = from_anzsic,
                       values_from = value) %>%
    dplyr::mutate(fte = `Employed full-time`/`Employed total`)

  data %>%
    dplyr::left_join(ratio, by = "anzsic_division_code") %>%
    dplyr::mutate(adjust_fte = adjust_jobs * fte) %>%
    dplyr::select(lga, anzsic_division_code, adjust_fte)
}
